...
## V3.3 · YouTube owner connection and guarded delivery

Read [the iPhone YouTube connection guide](docs/CONNECT_YOUTUBE_IPHONE.md). **Owner OAuth is not connected until the user authorizes and stores their credentials in GitHub repository Actions secrets.** The exact intended channel ID is pinned by `YT_CHANNEL_ID`. Once connected, daily reviewed-by-machine MP4s may be staged **private**, only to that channel. Public automation needs separately enabled feature flags, owner rights attestation and official GitHub release provenance. An unverified API project can be forced private by YouTube even when public is requested; the bot reads actual server privacy after upload. Uploads carry a unique `VELORA-ID` marker and are checked against the channel's recent uploads before any POST to reduce duplicates on retries. The bot never commits private video IDs or OAuth credentials. Read-only YouTube Analytics can connect using the same owner's refresh token and scopes and feeds cautious experiments on mature published videos. Source fact-check and audiovisual rights are not magically guaranteed by tests; unverified news remains private.
